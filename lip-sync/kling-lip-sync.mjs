...
Script to batch-generate talking lip-sync avatar videos with the Kling API v1 by useapi.net 🚀
Uses the asynchronous avatars/video endpoint (POST /avatars/video): an avatar source
(avatarId or imageUrl) plus an audio source (text + speakerId, or audioUrl).
For more details visit https://useapi.net/docs/api-kling-v1/post-kling-avatars-video
...
Usage: node kling-lip-sync.mjs <API_TOKEN> <EMAIL> [PROMPTS_FILE]
...
Each prompt needs one avatar source — avatarId (saved via POST /avatars) OR imageUrl (an
image URL uploaded via POST /assets) — and one audio source — text (with a required speakerId
from GET /tts/voices) OR audioUrl. List voices/emotions at GET /tts/voices.

Example:
--------

node kling-lip-sync.mjs user:1234-abcdefhijklmnopqrstuv my@email.com
...
// Constants
const RESULTS_FILE = 'kling_lipsync_results.txt';
const ERRORS_FILE = 'kling_lipsync_errors.txt';
...
const urlAvatarsVideo = 'https://api.useapi.net/v1/kling/avatars/video';
...
// Submit a single avatar video prompt. Returns { status, taskId }.
async function submitVideo(apiToken, email, prompt, index) {
    const { avatarId, imageUrl, audioUrl, text, speakerId, emotion, speed, mode } = prompt;

    const label = text ?? '(audioUrl)';

    console.log(`🚀 avatars/video » Prompt #${index} • ${email} • ${avatarId ? `avatarId ${avatarId}` : 'imageUrl'} …`);
    ...
    const createResponse = await fetch(urlAvatarsVideo, {
        method: 'POST',
        headers: {
            'Accept': 'application/json',
...
async function download(apiToken, email) {
    ...
    try {
        ...
        for (const line of lines) {
            ...
            const videoFilename = `kling_lipsync_${taskId}.mp4`;
...
async function main() {
    ...
    if (!apiToken || !email) {
        console.error('Usage: node kling-lip-sync.mjs <API_TOKEN> <EMAIL> [PROMPTS_FILE]');
        process.exit(1);
    }
...
async function execute(apiToken, email, promptFile) {
    ...
    // Parameters accepted by this script for the avatars/video endpoint.
    // See https://useapi.net/docs/api-kling-v1/post-kling-avatars-video for the full parameter set.
    const supportedParams = ['avatarId', 'imageUrl', 'audioUrl', 'text', 'speakerId', 'emotion', 'speed', 'mode'];
    ...
    for (let i = 1; i <= prompts.length; i++) {
        ...
        const { avatarId, imageUrl, audioUrl, text, speakerId } = prompt;
        ...
        if (!avatarId && !imageUrl)
            warnings.push(`⚠️  Please specify an avatar source (avatarId or imageUrl). Prompt ${i}`);

        if (!audioUrl && !text)
            warnings.push(`⚠️  Please specify an audio source (audioUrl or text). Prompt ${i}`);

        if (text && !speakerId)
            warnings.push(`⚠️  text requires a speakerId (list voices at GET /tts/voices). Prompt ${i}`);

        if (text && text.length > 5000)
            warnings.push(`⚠️  text exceeds 5000 characters. Prompt ${i}`);
    }
